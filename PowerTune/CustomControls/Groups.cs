...
public partial class Groups : ItemsControl {
    const string PartDescriptionPresenter = "DescriptionPresenter";
    ContentPresenter? _descriptionPresenter;
    Groups? _settingsGroup;
    ...
    static void OnDescriptionChanged(DependencyObject d, DependencyPropertyChangedEventArgs e) => ((Groups)d).Update();

    // Event handler for the IsEnabledChanged event
    void SettingsGroup_IsEnabledChanged(object sender, DependencyPropertyChangedEventArgs e) => SetEnabledState();

    // Set the enabled state of the control based on the IsEnabled property
    void SetEnabledState() => VisualStateManager.GoToState(this, IsEnabled ? "Normal" : "Disabled", true);
